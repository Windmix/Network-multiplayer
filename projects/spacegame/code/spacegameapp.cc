...
SpaceGameApp::Run()
{
    ...
    //join server spawning ships
 /*   Protocol::Message joinMsg;
    joinMsg.type = Protocol::Join;
    client.SendMsg(&joinMsg, sizeof(joinMsg));*/
    Core::CVar* r_spawn_ship = Core::CVarCreate(Core::CVarType::CVar_Int, "r_spawn_ship", "0");
    ...
    while (this->window->IsOpen())
	{
        ...
        int spawnShip = Core::CVarReadInt(r_spawn_ship);

        //Logic to spawn ships based on user input
        if (spawnShip > 0)
        {
            Protocol::Message joinMsg2;
            joinMsg2.type = Protocol::Join;
            //Protocol::CreateMessage(joinMsg2, joinMsg2.type);
            client.SendMsg(&joinMsg2, sizeof(joinMsg2));
            Core::CVarWriteInt(r_spawn_ship, 0);
        }
        std::string shipStrPos;
        for (auto& [id, client] : server.ClientShips)
        {
            if (client.ship)
            {
                client.ship->Update(dt);
                client.ship->CheckCollisions();
                RenderDevice::Draw(client.ship->model, client.ship->transform);

                shipStrPos = std::to_string(client.ship->position.x) +
                    " , " + std::to_string(client.ship->position.y) +
                    " , " + std::to_string(client.ship->position.z);

                Debug::DrawDebugText(shipStrPos.c_str(), client.ship->position + glm::vec3(0.0f, 2.0f, 0.0f), { 1,0,0,1 });
                Debug::DrawDebugText(std::to_string(client.myPlayerId).c_str(), client.ship->position + glm::vec3(0.0f, 2.1f, 0.0f), { 1,1,0,1 });
            }
        }
        ...
        //ebug::DrawDebugText(shipStrPos.c_str(), ship.position, {1,0,0,1});
...
SpaceGameApp::RenderUI()
{
	if (this->window->IsOpen())
	{
        ...
        Core::CVar* r_spawn_ship = Core::CVarGet("r_spawn_ship");
        static int rSpawnShip = Core::CVarReadInt(r_spawn_ship);
        if (ImGui::InputInt("Spawn Ship", &rSpawnShip))
        {
            Core::CVarWriteInt(r_spawn_ship, rSpawnShip);
        }

        //todo____ create a spawner ui for ships and then test networking;
        ImGui::End();
