...
SpaceShip::SpaceShip()
{
   ...
   /* this->particleCanonLeft = new ParticleEmitter(numParticles);
    this->particleCanonLeft->data = {
        .origin = glm::vec4(this->position + (vec3(this->transform[2]) * cannonOffset),1),
        .dir = glm::vec4(glm::vec3(-this->transform[2]), 0),
        .startColor = glm::vec4(0.38f, 0.76f, 0.95f, 1.0f) * 2.0f,
        .endColor = glm::vec4(0,0,0,1.0f),
        .numParticles = numParticles,
    ...
    this->particleCanonRight = new ParticleEmitter(numParticles);
    this->particleCanonRight->data = this->particleCanonLeft->data;

    ParticleSystem::Instance()->AddEmitter(this->particleCanonLeft);
    ParticleSystem::Instance()->AddEmitter(this->particleCanonRight);*/
...
SpaceShip::Update(float dt)
{
    ...
    if (kbd->held[Key::W])
    {
        Protocol::ClientInputData input{};
